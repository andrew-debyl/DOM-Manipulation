...
async function main() {
  ...
  //getting user data in a promise
  const userDataInPromise = ids.map((id) => fetchUserData(id))

  //getting user data without promise
  const users = await Promise.all(userDataInPromise);

  //mapping over all the users and turn it into html
